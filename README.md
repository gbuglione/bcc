...
In addition, IMO it's not clear why reversing a withdrawal should first result in adding freezed funds to an account, since such funds would not be usable.
...
Where deemed useful (e.g. locked account), certain invariants are enforced at the typesytem level, so that writing a program that changes the balance of a frozen account would be an invalid Rust program and rejected by the compiler.
